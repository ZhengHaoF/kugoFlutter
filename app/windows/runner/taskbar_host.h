...
class TaskbarHost {
 public:
  ...
  // 'next'). Must stay stable - the shell keys THBN_CLICKED by iId.
  ...
  void SyncButtons(bool has_track, bool is_playing);
 ...
 private:
  ...
  void ApplyButtons();
  void ApplyProgress();
  static HICON MakeGlyphIcon(int kind);  // 0 prev, 1 play, 2 pause, 3 next
  void DestroyIcons();
  void CreateIcons();
