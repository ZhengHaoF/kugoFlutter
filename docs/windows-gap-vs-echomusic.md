...
**本轮相对初版的实质变化**：P0 底线能力（SMTC / 托盘 / 关闭到托盘）与 Thumbar（**含收藏钮**）/ 任务栏进度条已落地。Windows 现在可以当「能后台驻留 + 有系统媒体控制」的播放器用；剩余缺口集中在**全局快捷键、独立窗口形态、窗口材质/状态、自启与电源**。
...
| **Thumbar 播控**（上一曲 / 播停 / 下一曲 / **收藏**） | `windows/runner/taskbar_host.cpp` + `shared/taskbar/taskbar_bridge.dart` |
...
| --- | --- | --- | --- |
| 5 | **Thumbar 播控按钮** | `main/thumbar.ts`：上一曲 / 播放暂停 / 下一曲 + 收藏；纯 JS 生成抗锯齿图标 | ✅ **已落地 4/4**：`TaskbarHost`（`ITaskbarList3`）上一曲 / 播停 / 下一曲 / **收藏**（心形描边↔实心）；GDI 抗锯齿 glyph；`TaskbarCreated` 重建后 Refresh |
...
- **Thumbar 四钮**：`favorite` 事件走 `taskbar_bridge` → `likes_controller`；心形 filled/outline 双 glyph。曲名 tooltip 仍未跟播放态（属⑦封面预览配套）。
...
| Thumbar | 悬停任务栏图标出现上一曲 / 播停 / 下一曲 / 收藏，点击有效；收藏态心形与应用内一致 |
| 任务栏进度 | 播放中任务栏图标有进度，暂停态可区分；设置可关 |
...
| 文档 | 内容 |
| --- | --- |
| `gap-vs-echomusic.md` | 全端业务能力差距（搜索多维、歌单 CRUD、分享、倍速 UI 等） |
| `设计方案.md` | 移动端原方案 + 实现对照；Windows 系统集成细节以本文为准（SMTC 走 `audio_service_win`） |
