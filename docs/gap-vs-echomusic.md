...
当前排期主线：工期规划.md Phase 5 · 酷狗音源抽象（多音源阶段 A，约 2 周）
  —— 见 多音源接入方案.md §8.1；与本表产品缺口无关，优先于产品二期。

产品二期 backlog（未排期，多音源 A 完成后另立 M6）：
  倍速 UI（0.5d）/ 自建歌单 CRUD（5d）/ 逐字歌词 UI + 音译（2d）/ 分享（1d）
  另：歌单封面/排序、MV、评论写入、听歌偏好、登录设备管理
```
