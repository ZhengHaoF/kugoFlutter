...
| # | 问题 | 位置 | 说明 |
| --- | --- | --- | --- |
| 1 | 「本地音乐」点击无反应 | `lib/features/profile/profile_page.dart:173-179` | `onTap: () {}` 空实现，是死入口 |
| 2 | 「下载管理」点击无反应 | `lib/features/profile/profile_page.dart:180-186` | `onTap: () {}` 空实现，是死入口 |
| 3 | 「定时停止 / 音质设置 / 关于」不可点 | `profile_page.dart:202/203/209` | `_LinkTile` 未传 `onTap`，无跳转。功能已有（在设置页内），只是入口没接线 |
...
```
第一步（1 天内可清）：修 profile 死入口 + 假数字 + 倍速接线
